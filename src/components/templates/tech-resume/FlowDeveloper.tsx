...
export default function FlowDeveloper({
  ...
  return (
    <article
      className={`single-page-resume bg-white text-neutral-900 min-h-[297mm] ${config.padding} font-sans shadow-none leading-normal selection:bg-indigo-100 w-full`}
      style={
        {
          fontFamily:
    ...
    >
      {/* Sleek FlowCV-Style Modern Header */}
      ...
      <div className="space-y-5">
        ...
        {validExperience.length > 0 && (
          <section>
            <h2 className="text-xs font-bold uppercase tracking-wider text-neutral-900 mb-3 flex items-center gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-neutral-900" />
              Experience
            </h2>
        ...
        {(() => {
          ...
          const renderLanguages = (inColumn = false) =>
            validLanguages.length > 0 && (
              <section className={inColumn ? 'avoid-break' : 'pt-2 border-t border-neutral-200 avoid-break'}>
                ...
                <div className="flex flex-wrap items-center gap-3 text-xs">
                  {validLanguages.map((lang, idx) => (
                    <span key={idx} className="text-neutral-700 inline-flex items-center gap-1">
          ...
          if (secondaryItems.length === 2) {
            ...
            return (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 items-start pt-1">
                <div className="space-y-4">{leftItem.render(true)}</div>
                <div className="space-y-4">{rightItem.render(true)}</div>
              </div>
            );
          }
